...
### แกลเลอรีรวมศูนย์ — รูปงานทั้งหมดในการ์ดเดียว — เพิ่ม 27 มิ.ย. 2569
เวิร์กโฟลว์จริง: น้องอัปงานขึ้น **Google Drive แล้วแปะลิงก์** เข้า Trello (attachment/desc/**คอมเมนต์**) · พี่อยากเห็นรวมที่เดียวไม่ต้องคลิกออก
- `content_trello` ส่ง **`images[]`** = `{thumb,full,open,kind}` รวมทุกรูป: รูปแนบ Trello (proxy, ได้หลายใบ) + **ไฟล์ Drive จาก attachment+desc+คอมเมนต์** (thumbnail สาธารณะ) · cap 8/การ์ด
- `open` ของ Drive ชี้ไฟล์จริง (`/file/d/{id}/view`) ไม่ใช่ thumbnail → ถ้า thumbnail โดนจำกัดสิทธิ์ยังกดเปิดได้
- หน้าเว็บ: โมดัลโชว์ **แกลเลอรี grid** (รูปเดียว=เต็มกว้าง · หลายรูป=2 คอลัมน์) · รูปที่โหลดไม่ได้ → `_cnImgFb()` แทนด้วยกล่อง "เปิดไฟล์ Drive" (ลิงก์ยังกดได้)
- `links[]` ตัดไฟล์รูป Drive ออก (กลายเป็น images แล้ว) เหลือ Canva/YouTube/FB/โฟลเดอร์ · ดึงจากคอมเมนต์ด้วย (cap 14)
- ผลจริง: 503 ใบมีรูป · **465 ใบมีหลายรูป** (เฉลี่ย 2-3 รูป สูงสุด 8) · ข้อจำกัด: ไฟล์ Drive ที่ทีมตั้ง "จำกัดสิทธิ์" thumbnail ไม่ขึ้น (โชว์ปุ่มเปิดไฟล์แทน) → แนะนำตั้งแชร์ "ทุกคนที่มีลิงก์ดูได้"
- **Lightbox** (`_cnLightbox`/`_CN_GAL`): กดรูปในแกลเลอรี → ซูมเต็มจอในแดชบอร์ด (ไม่เปิดแท็บใหม่) · ‹ › เลื่อนทุกรูป · ✕/คลิกพื้นหลัง/Esc ปิด · ลูกศรซ้าย-ขวาเลื่อน · รูปเต็มที่โหลดไม่ได้ → ลิงก์ "เปิดไฟล์ Drive" (`_cnLbFb`)

### รีวิว+เก็บ UX โมดัลการ์ด — เพิ่ม 27 มิ.ย. 2569
รีวิวพบ "การ์ดข้อมูลเยอะ" รกและยาวเกิน (desc 14k ตัว → โมดัลสูง 25,000px) แก้ 4 จุด:
1. **ย่อเนื้อหา/คอมเมนต์ยาว** (`_cnClamp`/`_cnUnclamp`): desc >700 ตัว หรือคอมเมนต์ >500 ตัว → กล่อง max-height + ปุ่ม "ดูเพิ่ม/ย่อ" (โมดัลสั้นลง ~84%)
2. **ซ่อนแถว/ป้ายว่าง**: ไม่ render `ผู้ดูแล/รูปแบบ/หัวข้อ/ป้ายแผนก-หัวข้อ` เมื่อว่าง · ตัดแถว `สถานะ` ซ้ำ (มีใน header แล้ว)
3. **รวมปุ่มลิงก์ซ้ำ**: group ตาม kind · kind ที่มีหลายลิงก์ → ป้าย "ดูโพสต์ (N)" + เลข 1..N (เลี่ยงกำแพงปุ่ม)
4. **ปุ่มอนุมัติ/ตีกลับเฉพาะคอลัมน์รีวิว** (`เซินรีวิว/ตองตรวจ/แก้ไข`) · คอลัมน์อื่นโชว์แค่กล่องคอมเมนต์

### เดือนที่จะโพส + ดึงลิสต์เดือน — เพิ่ม 27 มิ.ย. 2569
พบว่าทีมจัดงานหลายแกน: บอร์ดมี **ลิสต์ตามเดือน** ("กุมภาพันธ์/มีนาคม/เมษายน/พฤษภาคม 2569", โปรโมชั่นรายเดือน) = แผนคอนเทนต์ว่าจะโพสเดือนไหน — เดิม `LIST_MAP` ดึงแต่ลิสต์สเตจ จึง **มองข้ามลิสต์เดือน 190 ใบ**
- `content_trello`: ดึงลิสต์เดือนเข้ามาด้วย (`listMonthIso` parse ชื่อลิสต์ → map คอลัมน์ = "วางแผน") · field ใหม่ `post_month` (yyyy-mm) + `post_month_label` ("มี.ค. 69") + `post_date_src` (list/due/code)
- ที่มาเดือน (เรียงความชัด): **ชื่อลิสต์เดือน** → due date → โค้ด `[DDMMYY]` ต้นชื่อ (`dateFromName`, YY>50=พ.ศ.)
- หน้าเว็บ: **ป้ายเดือน** บนการ์ด (teal 📅) + แถว "เดือนที่จะโพส" ในโมดัล + **ตัวกรองเดือน** (`_cnBoardF.month`) บนบอร์ด
- ผลจริง: 194 ใบมีเดือน (ก.พ.39·มี.ค.90·เม.ย.34·พ.ค.20·มิ.ย.10) · การ์ดรวม 535→725
- ~~ข้อจำกัด: ลิสต์เดือนกองที่ "วางแผน"~~ → **แก้แล้ว: แยกเป็นหน้า "ปฏิทินคอนเทนต์"**

### หน้า "ปฏิทินคอนเทนต์" (cnplan) — จัดกลุ่มตามเดือน — 27 มิ.ย. 2569
พี่เซินเลือกให้แยกหน้า (ไม่ยัดลง kanban สเตจ):
- `mountCnBoard`: **กรองการ์ด `post_date_src==='list'` ออก** → kanban กลับมาโฟกัสสเตจ (~535 ใบ)
- `mountCnPlan` (เมนู "ปฏิทินคอนเทนต์", เดิมเป็น mock day-calendar): เขียนใหม่เป็น **section ตามเดือน** (เรียง yyyy-mm) แต่ละเดือน = grid การ์ด (รูป+ชื่อ+คนทำ+สถานะ) คลิกเปิดโมดัล (`cnPiece`) · จัดกลุ่มด้วย `post_month || date.slice(0,7)`
- ผลจริง: 194 คอนเทนต์ระบุเดือน · 6 เดือน (ก.พ. 39 · มี.ค. 90 · เม.ย. 34 · พ.ค. 20 · มิ.ย. 10)
- kanban ยังมี badge เดือน + ตัวกรองเดือน (สำหรับการ์ดสเตจที่มี due/โค้ด)
