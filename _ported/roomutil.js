...
var RU_PROVIDER_TARGET = 26; // นัด/สัปดาห์ ที่เทรนเนอร์ 1 คนรับไหวแบบยั่งยืน (full load)
...
var RU_BACKEND = {
  load: function () {
    return Promise.all([
      ...
      ruFetchType('jera.provider_load.updated'),
    ]).then(function (r) {
      ...
      var providers = r[3].map(function (p) {
        return {
          uuid: p.staff_uuid || p.entity_id, name: p.staff_name || '-',
          branch_id: p.branch_id || 'SLY',
          sessions_week: ruNum(p.sessions_per_week), hours_week: ruNum(p.hours_per_week),
          sessions_30d: ruNum(p.sessions_30d), recall_30d: ruNum(p.recall_30d),
          window_days: ruNum(p.window_days) || 30,
        };
      ...
      return { rooms: rooms, courses: courses, appts: appts, providers: providers, branches: branches };
    });
  },
};
...
function RU_RUN() {
  ...
  function compute() {
    ...
    // บุคลากร (โหลดจริง 30 วันล่าสุด) — คอขวด
    var provs = (_d.providers || []).filter(function (p) { return !bf || p.branch_id === bf; });
    var activeProv = provs.filter(function (p) { return p.sessions_30d > 0; });
    activeProv.sort(function (a, b) { return b.sessions_week - a.sessions_week; });
    var staffLoad = activeProv.reduce(function (s, p) { return s + p.sessions_week; }, 0); // นัดจริง/สัปดาห์ (throughput)
    var staffCap = activeProv.length * RU_PROVIDER_TARGET;
    var staffUtil = staffCap > 0 ? staffLoad / staffCap * 100 : 0;
    var top3 = activeProv.slice(0, 3).reduce(function (s, p) { return s + p.sessions_week; }, 0);
    var top3Share = staffLoad > 0 ? top3 / staffLoad * 100 : 0;
    var roomUtilReal = slotsPerWeek > 0 ? staffLoad / slotsPerWeek * 100 : 0;
    var staffSpare = activeProv.reduce(function (s, p) { return s + Math.max(0, RU_PROVIDER_TARGET - p.sessions_week); }, 0);
    ...
    return {
      ...
      providers: activeProv, staffLoad: staffLoad, staffCap: staffCap, staffUtil: staffUtil,
      top3Share: top3Share, roomUtilReal: roomUtilReal, staffSpare: staffSpare,
    };
  }
  ...
  function render() {
    ...
    $id('ru-content').innerHTML =
      renderCapacity(c) + renderBottleneck(c) + renderDow(c) + renderRoomTypes(c) + renderCourses(c);
  ...
  function renderBottleneck(c) {
    if (!c.providers || !c.providers.length) {
      return '<div class="sec-title">🔧 คอขวด (Bottleneck)</div><div class="cap-panel"><div class="note">ยังไม่มีข้อมูลโหลดบุคลากร (jera.provider_load.updated) — จะแสดงเมื่อ publisher รันรอบถัดไป</div></div>';
    }
    // หา constraint ที่ตึงสุด: ห้อง vs บุคลากร (เทียบ % การใช้จาก throughput จริง)
    var roomU = c.roomUtilReal, staffU = c.staffUtil;
    var bottleneck = staffU >= roomU ? 'บุคลากร' : 'ห้อง';
    var bColor = Math.max(roomU, staffU) >= RU_RED ? 'r' : Math.max(roomU, staffU) >= RU_YELLOW ? 'y' : 'g';
    var pipeAdd = c.pipeWeeks >= 999 ? 0 : c.pipePerWeek;
    var staffUProj = c.staffCap > 0 ? (c.staffLoad + pipeAdd) / c.staffCap * 100 : 0;

    function capCard(cls, label, capVal, util, sub) {
      var u = Math.min(100, util);
      return [
        '<div class="m" style="padding:12px">',
        '  <div class="k">' + esc(label) + '</div>',
        '  <div class="x" style="display:flex;justify-content:space-between;align-items:baseline">' + capVal + ' <span style="font-size:12px;color:' + (util >= RU_RED ? '#DC2626' : util >= RU_YELLOW ? '#D97706' : '#059669') + '">' + fmtPct(util) + '</span></div>',
        '  <div class="bar" style="height:8px;margin-top:6px"><div class="fill ' + statusClass(util) + '" style="width:' + u + '%"></div></div>',
        '  <div class="sub" style="margin-top:5px">' + esc(sub) + '</div>',
        '</div>',
      ].join('');
    }

    var rows = c.providers.map(function (p) {
      var u = RU_PROVIDER_TARGET > 0 ? p.sessions_week / RU_PROVIDER_TARGET * 100 : 0;
      var st = u >= RU_RED ? '<span class="pill r">เต็ม</span>' : u >= RU_YELLOW ? '<span class="pill y">หนัก</span>' : '<span class="pill g">ว่าง</span>';
      var head = Math.max(0, RU_PROVIDER_TARGET - p.sessions_week);
      return '<tr><td>' + esc(p.name.replace(/^\[|\].*$/g, '').slice(0, 24) || p.name) + '</td>' +
        '<td class="num"><b>' + fmtInt(p.sessions_week) + '</b></td>' +
        '<td class="num">' + fmtInt(p.hours_week) + '</td>' +
        '<td class="num">' + fmtInt(p.recall_30d / (p.window_days || 30) * 7) + '</td>' +
        '<td class="num">' + fmtPct(u) + '</td>' +
        '<td class="num">' + fmtInt(head) + '</td>' +
        '<td>' + st + '</td></tr>';
    }).join('');

    var concWarn = c.top3Share >= 50
      ? '<div class="note" style="color:#B45309;border-top:1px dashed var(--border);padding-top:9px;margin-top:11px">⚠️ <b>ความเสี่ยงกระจุก:</b> เทรนเนอร์ 3 คนแรกแบก <b>' + fmtPct(c.top3Share) + '</b> ของงาน — ถ้าคนใดลาออกจะเสียกำลังส่งมอบทันที</div>'
      : '';

    var verdict = bottleneck === 'บุคลากร'
      ? 'คอขวด = <b>บุคลากร</b> (ใช้ ' + fmtPct(staffU) + ') · ห้องเหลือเยอะ (ใช้แค่ ' + fmtPct(roomU) + ') → ขยาย "คน" ก่อน ไม่ใช่ห้อง'
      : 'คอขวด = <b>ห้อง</b> (ใช้ ' + fmtPct(roomU) + ') · บุคลากรยังรับได้ → พิจารณาเพิ่มพื้นที่';

    return [
      '<div class="sec-title">🔧 คอขวด (Bottleneck) — โหลดจริง 30 วันล่าสุด</div>',
      '<div class="cap-panel">',
      '  <div class="gauge-row" style="margin-bottom:12px"><span class="badge ' + bColor + '">' + (bColor === 'r' ? '⚠ ' : bColor === 'y' ? '◐ ' : '✓ ') + verdict + '</span></div>',
      '  <div class="cap-meta" style="grid-template-columns:repeat(3,1fr);margin-top:0">',
      capCard('', 'ห้อง', fmtInt(c.slotsPerWeek), roomU, 'รับได้/สัปดาห์ · ส่งจริง ' + fmtInt(c.staffLoad)),
      capCard('', 'บุคลากร (' + c.providers.length + ' คน)', fmtInt(c.staffCap), staffU, 'รับไหว/สัปดาห์ @' + RU_PROVIDER_TARGET + '/คน'),
      capCard('', 'รวมคอร์สค้างแล้ว', '', staffUProj, 'โหลดคน +คอร์สค้าง = ' + fmtPct(staffUProj)),
      '  </div>',
      '  <div class="note" style="margin-top:11px">ช่องว่างบุคลากรที่เหลือ ≈ <b>' + fmtInt(c.staffSpare) + ' นัด/สัปดาห์</b>' + (pipeAdd > 0 ? ' · คอร์สค้างต้องการ ≈ ' + fmtInt(pipeAdd) + ' นัด/สัปดาห์ → ' + (c.staffSpare >= pipeAdd ? 'พอรับถ้ากระจายโหลดไปคนที่ว่าง' : '<b style="color:#B91C1C">ไม่พอ ต้องจ้างเพิ่ม</b>') : '') + '</div>',
      concWarn,
      '  <div class="table-wrap" style="margin-top:12px"><table class="data-table"><thead><tr><th>เทรนเนอร์</th><th class="num">นัด/สัปดาห์</th><th class="num">ชม./สัปดาห์</th><th class="num">recall/สัปดาห์</th><th class="num">% เต็ม</th><th class="num">ว่าง</th><th>สถานะ</th></tr></thead><tbody>' + rows + '</tbody></table></div>',
      '</div>',
    ].join('');
  }
